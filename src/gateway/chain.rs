...
pub async fn socks5_connect_through(
    ...
    if resp_header[0] != 0x05 || resp_header[1] != 0x00 {
        return Err(std::io::Error::new(
            std::io::ErrorKind::ConnectionRefused,
            format!("SOCKS5 connection failed with reply code: {}", resp_header[1]),
        ));
    }
...
pub async fn intercept_socks5_request(stream: &mut TcpStream) -> std::io::Result<(String, u16)> {
    ...
    if req_header[0] != 0x05 || req_header[1] != 0x01 { // Only support CONNECT
    ...
    match atyp {
        ...
        _ => return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "Unknown ATYP")),
    }
