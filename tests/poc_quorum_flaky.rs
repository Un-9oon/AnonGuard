...
#[tokio::test]
async fn poc_quorum_with_divergent_relay_registration() {
    let now = current_timestamp_secs();
    let rk1 = SigningKey::generate(&mut OsRng);
    let nonce1 = solve_pow_bounded("relay-a", now, 8).unwrap();
    let mut d1 = RelayDescriptor::new(
        "relay-a".into(),
        "1.2.3.4".into(),
        9001,
        [7; 32],
    ...
    d1.sign_with_key(&rk1);

    let rk2 = SigningKey::generate(&mut OsRng);
    let nonce2 = solve_pow_bounded("relay-b", now, 8).unwrap();
    let mut d2 = RelayDescriptor::new(
        "relay-b".into(),
        "1.2.3.5".into(),
        9002,
        [8; 32],
    ...
    a2.register_relay(d2.clone()).await.unwrap();
    ...
    for _i in 0..n {
        ...
        let mut m = fetch("127.0.0.1:19303", &v1).await;
        let other = fetch("127.0.0.1:19304", &v2).await;
        m.merge_signatures_from(&other);
        if m.verify_quorum(&trusted, 2, current_timestamp_secs()) {
            ok += 1;
        }
    }
    println!("Divergent registration -> 2-of-2 quorum reached in {ok}/{n} rounds");
    assert!(
        ok >= 18,
        "Expected quorum in at least 18/20 rounds with reconciliation, got {ok}/{n}"
    );
}
